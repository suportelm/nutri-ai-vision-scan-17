...
const Stats = () => {
  ...
  const subscriptionPlans = [{
    ...
    features: ['Escaneamento básico de alimentos', 'Estatísticas semanais', 'Histórico de 7 dias', 'Acompanhamento de calorias'],
    limits: {
      scans: 5,
      history: '7 dias',
      reports: false,
    ...
    features: ['Todos os recursos Premium', '2 meses grátis', 'Suporte prioritário', 'Beta features exclusivos'],
    limits: {
      scans: 'unlimited' as const,
      history: 'Completo',
      reports: true,
  ...
  return <div className="min-h-screen bg-background text-foreground pb-20">
      ...
      <div className="px-6 space-y-6">
        <PremiumBanner onShowReports={() => setShowReports(true)} onShowSubscription={() => setShowSubscription(true)} />
      ...
      <ReportsModal isOpen={showReports} onClose={() => setShowReports(false)} period={timeframe} />

      {showSubscription && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-background rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-border">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-center">Escolha seu Plano</h2>
                  <p className="text-muted-foreground text-center">Desbloqueie todo o potencial do FoodCam AI</p>
                </div>
                <button className="text-muted-foreground hover:text-foreground text-2xl" onClick={() => setShowSubscription(false)}>
                  ×
                </button>
              </div>
            </div>
            <div className="p-6">
              <div className="grid md:grid-cols-3 gap-6">
                {subscriptionPlans.map(plan => <SubscriptionCard key={plan.id} plan={plan} currentPlan="free" onSelectPlan={handleSelectPlan} />)}
              </div>
            </div>
          </div>
        </div>}
